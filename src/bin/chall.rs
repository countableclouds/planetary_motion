...
use planetary_motion::ROCKET_FACTOR;
...
fn main() {
    ...
    println!("{}", other_planet.velocity.y * other_planet.mass * distance);

    let mut window = Window::new("Simulation! 😎");
    window.set_light(Light::StickToCamera);
    window.set_background_color(1.0, 1.0, 1.0);

    let eye = Point3::new(0., 1000., 0.);
    ...
    while window.render_with_camera(&mut arc_ball) {
        ...
        // println!("force: {:?}", force);
        let rocket_force = Planet::rocket_force(&other_planet, &center_planet);
        ...
        other_planet.velocity += rocket_force * DELTA_T / other_planet.mass;
    ...
    fs::write(&format!("csv/challenge_{:.2}.csv", ROCKET_FACTOR), data)
    ...
    Command::new("ffmpeg")
        .args([
            ...
            "-c:a",
            "copy",
            &format!("videos/challenge_motion_{:.2}.mp4", ROCKET_FACTOR),
        ])
        .output()
        .expect("failed to execute process");
